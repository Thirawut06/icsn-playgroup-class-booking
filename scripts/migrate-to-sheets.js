...
async function runMigration() {
  ...
  // PHASE 4: ADMIN DEDUCT BOOKINGS
  console.log("\n--- PHASE 4: ADMIN DEDUCT BOOKINGS ---");
  const { data: adminTx, error: adminTxError } = await supabase.from('credit_transactions').select('*').eq('action_type', 'admin_deduct_booking').order('created_at', { ascending: true });
  if (adminTx) {
    for (let i = 0; i < adminTx.length; i++) {
      console.log(`⏳ [${i + 1}/${adminTx.length}] Admin Deduct: ${adminTx[i].id}`);
      try {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/append-to-sheets`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${SERVICE_KEY}` },
          body: JSON.stringify({ form_type: 'credit_transaction', parentId: adminTx[i].parent_id, transactionId: adminTx[i].id })
        });
        const result = await res.json();
        if (result.status !== 'success') console.error('Failed:', result.message);
      } catch (err) {
