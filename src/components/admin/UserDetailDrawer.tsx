      ...
      <div className={`fixed inset-y-0 right-0 w-full max-w-2xl bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800">รายละเอียดผู้ใช้งาน</h2>
              <p className="text-sm text-gray-500">ข้อมูลทั้งหมดที่เกี่ยวข้องกับผู้ใช้งานนี้</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    ...
                    <p className="font-medium text-gray-800">{formatDateShort(data.created_at)}</p>
                  </div>
                </div>
              </div>
                    ...
                    {data.children.map((child: any) => (
                      <div key={child.id} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            <p className="font-bold text-gray-800 text-lg">{child.nickname}</p>
                        ...
                        {child.parent_photo_url && (
                          <div className="mb-4">
                            <p className="text-xs text-gray-500 mb-1">รูปถ่ายคู่ผู้ปกครอง</p>
                            <img src={child.parent_photo_url} alt="Parent & Child" className="h-24 w-auto rounded-lg object-cover border border-gray-200" />
                          </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          ...
                          <div className="col-span-2">
                            <p className="text-gray-500">อาหารที่แพ้</p>
                            <p className="font-medium text-red-600">{child.food_allergy || 'ไม่มี'}</p>
                          </div>
                          {child.special_info && (
                            <div className="col-span-2">
                              <p className="text-gray-500">ข้อมูลพิเศษ</p>
                              <p className="font-medium">{child.special_info}</p>
                            </div>
                          )}
                          <div className="col-span-2 flex gap-4 mt-2">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${child.media_perm ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700'}`}>
                ...
                {data.packages && data.packages.length > 0 ? (
                  <div className="space-y-3">
                    {data.packages.map((pkg: any) => (
                      <div key={pkg.id} className="flex items-center justify-between p-3 bg-blue-50/50 rounded-xl border border-blue-100">
                        <div>
                          <p className="font-bold text-gray-800">{pkg.type === 'trial' ? 'ทดลองเรียน (Trial)' : 'แพ็กเกจเรียน (Payment)'}</p>
                          <p className="text-xs text-gray-500">ซื้อเมื่อ: {formatDateShort(pkg.created_at)}</p>
                        </div>
                  ...
                  <p className="text-sm text-gray-500 text-center py-4 bg-gray-50 rounded-xl">ไม่พบประวัติแพ็กเกจ (Walk-in)</p>
                )}
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                      {data.credit_transactions.sort((a:any, b:any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 5).map((log: any) => (
                        <div key={log.id} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded-lg">
                          <div>
                            <p className="font-medium text-gray-700">{log.notes || log.action_type}</p>
              ...
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                ...
                {data.bookings && data.bookings.length > 0 ? (
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                    {data.bookings.sort((a:any, b:any) => new Date(b.session_date).getTime() - new Date(a.session_date).getTime()).map((booking: any) => (
                      <div key={booking.id} className="flex items-center justify-between p-3 border border-gray-100 rounded-xl">
                        <div>
                          <p className="font-bold text-gray-800">{formatDateShort(booking.session_date)}</p>
                          <p className="text-xs text-gray-500">{booking.sessions?.time_label || '09:30 - 11:30'}</p>
                        </div>
                        <div className="text-right">
                  ...
                  <p className="text-sm text-gray-500 text-center py-4 bg-gray-50 rounded-xl">ไม่พบประวัติการจอง</p>
                )}
              </div>
              ...
              <div className="bg-yellow-50/50 rounded-2xl p-5 border border-yellow-100 shadow-sm">
                ...
                <textarea
                  ...
                  className="w-full h-24 p-3 border border-yellow-200 rounded-xl bg-white text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400/50 resize-none"
                />
                <div className="flex justify-end mt-2">
                  <button
                    onClick={handleSaveNotes}
                    disabled={savingNotes}
                    className="flex items-center gap-1 bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition disabled:opacity-50"
                  >
            ...
            <div className="text-center text-gray-500 mt-10">ไม่พบข้อมูล</div>
          )}
        </div>
      </div>
    </>
